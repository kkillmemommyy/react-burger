...
export const OrderModal = ({ orderId, isLoading, isError }: Props) => {
  if (isLoading) {
    return (
    ...
    return (
      <div className={cls.wrap}>
        ...
        <p className={clsx(cls.error_text, 'text text_type_main-medium mt-4 mb-4')}>
          Во время оформления заказа что-то пошло не так, попробуйте повторить его позднее
        </p>
  ...
  return (
    <div className={cls.order_modal}>
      <p className={clsx(cls.title, 'text text_type_digits-large mb-8')}>{orderId}</p>
